...
def test_pipeline_outputs_only_a_validated_patch(tmp_path: Path):
    root = _python_project(tmp_path / "project")
    project = ProjectLoader().load(root)
    ...
    class FakeRunner:
        ...
        def run(self, *, workspace, prompt, artifact_dir):
            assert "Baseline validation" in prompt
...
def test_pipeline_does_not_emit_a_patch_for_clean_baseline(tmp_path: Path):
    root = _python_project(tmp_path / "clean-project")
    (root / "src" / "value.py").write_text("def value():\n    return 1\n", encoding="utf-8")
    project = ProjectLoader().load(root)

    class RunnerMustNotStart:
        def __init__(self, **_kwargs):
            raise AssertionError("Codex must not run for a clean baseline")
    ...
    result = DebuggingPipeline(
        settings=settings,
        validator=ProjectValidator(command_timeout=30),
        runner_factory=RunnerMustNotStart,
